...
class TestProcess:
    ...
    def test_initial_value(self, process, denstream_kwargs):
        ...
        assert type(process.th) is int
        assert process.th == 43200
        ...
        assert process.denstream.label == 0
        assert process.denstream.time == 0
        assert not process.denstream.initiated
        assert process.denstream.all_cases == set()
        assert process.denstream.stream_speed == 1000
        assert process.denstream.ncluster == 0
    ...
    def test_get_list_for_gp(self, process):
        case1 = Case('1')
        case2 = Case('2')
        case3 = Case('3')
        case4 = Case('4')
        ...
        case_list_before_cp = process.get_list_for_gp('4')
        assert case_list_before_cp == [case1, case2, case3]
    ...
    def test_del_cases(self, process):
        case1 = Case('1')
        case2 = Case('2')
        case3 = Case('3')
        case4 = Case('4')
    ...
    def test_g_pova(self, process, cases_list):
    ...
    def test_initialize_cdesf(self, process, cases_list):
        ...
        # these stopped because not working, trying to test denstream to understand better where is the error
        # assert process.denstream.all_cases == {'1', '2', '3'}
        # assert process.denstream.label == 1
        #p_mc = process.denstream.p_micro_clusters
        #assert p_mc
        #assert p_mc.n_feature == 2
    ...
    def test_set_case(self, process):
        ...
        # this case always makes set_case working improperly, due to timestamp, which is before cp
        # process.set_case('1', 'activityA', datetime(2015, 5, 10, 8, 00, 00), 0)
        # it returns false, but theoretically it should have been entered in initialize_cdesf
        # assert not process.gp_creation
