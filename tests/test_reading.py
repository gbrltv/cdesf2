...
def test_read_csv():
    ...
    assert isinstance(event_stream_test, log.EventStream)
...
def test_read_xes():
    ...
    assert isinstance(first_event["time:timestamp"], datetime)
    assert first_event["time:timestamp"] == datetime(
        2010, 12, 30, 11, 2, tzinfo=tzinfo)
    ...
    assert isinstance(last_event["time:timestamp"], datetime)
    assert last_event["time:timestamp"] == datetime(
        2011, 1, 12, 15, 44, tzinfo=tzinfo)
